...
class FullFinetuneRecipeSingleDevice(FTRecipeInterface):
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        # VQ learning
        self._freeze_checkpoint_params = cfg.freeze_checkpoint_params
        self._trainable_param_keys = cfg.trainable_param_keys
        self._wandb_watch_layers = cfg.wandb_watch_layers
        self._vq_loss_scale = cfg.vq_loss_scale
        self._disable_gradient = cfg.disable_gradient
    ...
    def _setup_model(
        ...
        if self._freeze_checkpoint_params:
            for name, param in model.named_parameters():
                if name in model_state_dict:
                    param.requires_grad = False

        if self._trainable_param_keys:
            for name, param in model.named_parameters():
                if any(k in name for k in self._trainable_param_keys):
        ...
        log.info(
            "Initialization precision validation is turned off. Please make sure model is initialized with correct precision."
        )
    ...
    def save_checkpoint(self, epoch: int) -> None:
        ...
        if epoch + 1 < self.total_epochs:
            ...
            if self._optimizer and not self._optimizer_in_bwd:
                ckpt_dict[utils.OPT_KEY] = self._optimizer.state_dict()
            elif self._optim_ckpt_wrapper:
                ckpt_dict[utils.OPT_KEY] = self._optim_ckpt_wrapper.state_dict()
        self._checkpointer._output_dir.mkdir(parents=True, exist_ok=True)
    ...
    def train(self) -> None:
        ...
        running_loss = 0
        running_ce_loss = 0
        running_commitment_loss = 0
        num_tokens = 0

        if self._wandb_watch_layers:
            wandb.watch(
                [self._model.layers[i] for i in self._wandb_watch_layers],
                log_freq=10,
                log="all",
            )
        ...
        for curr_epoch in range(self.epochs_run, self.total_epochs):
            ...
            for idx, batch in enumerate(self._dataloader):
                ...
                # vq_mask = labels != CROSS_ENTROPY_IGNORE_IDX
                # vq_mask = F.pad(vq_mask, (0, 1), value=False)

                vq_mask = labels != CROSS_ENTROPY_IGNORE_IDX

                with torch.set_grad_enabled(not self._disable_gradient):
                    logits, commitment_loss = self._model(
                        tokens,
                        mask=mask,
                        input_pos=input_pos,
                        return_vq_loss=True,
                ...
                logits = logits.contiguous().transpose(1, 2)

                # mask vq loss
                # commitment_loss = commitment_loss[..., :-1]
                commitment_loss = (vq_mask * commitment_loss).sum() / vq_mask.sum()

                # Compute loss
                ce_loss = self._loss_fn(logits, labels)
                loss = ce_loss + self._vq_loss_scale * commitment_loss

                loss = loss / self._gradient_accumulation_steps
                running_loss = running_loss + loss
                running_ce_loss = (
                    running_ce_loss + ce_loss / self._gradient_accumulation_steps
                )
                running_commitment_loss = (
                    running_commitment_loss
                    + commitment_loss / self._gradient_accumulation_steps
                )

                if not self._disable_gradient:
                    loss.backward()
                ...
                if (idx + 1) % self._gradient_accumulation_steps == 0:
                    ...
                    self._model.update_quantizer_weight()
                    self.global_step += 1

                    loss_to_log = running_loss.item()
                    ce_loss_to_log = running_ce_loss.item()
                    commitment_loss_to_log = running_commitment_loss.item()
                    ...
                    if self.global_step % self._log_every_n_steps == 0:
                        ...
                        log_dict = {
                            "loss": loss_to_log,
                            "ce_loss": ce_loss_to_log,
                            "vq/commitment_loss": commitment_loss_to_log,
                            "tokens_per_second_per_gpu": num_tokens / time_per_step,
                        }
                        if self._optimizer or self._optimizer_in_bwd:
